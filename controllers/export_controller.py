...
from models import Page, Project, ODataConnection
...
def _render_component_html(comp) -> str:
    ...
    style = (f"left:{comp.x}px;top:{comp.y}px;"
    ...
    if comp.type in ("textbox","numberbox","datepicker"):
        itype = {"textbox":"text","numberbox":"number","datepicker":"date"}.get(comp.type,"text")
        lbl   = f'<label style="font-size:11px;font-weight:600;display:block;margin-bottom:2px;">{p.get("label","")}</label>' if p.get("label") else ""
    ...
    if comp.type == "datagrid":
        ...
        rows = "".join("<tr>" + "".join(f"<td>{cell}</td>" for cell in row) + "</tr>"
                       for row in (p.get("rows") or []))
        return (f'<div id="comp_{comp.id}" class="dsb-comp" style="{style};overflow:auto;" {dtype}>'
    ...
    if comp.type == "groupbox":
        return (f'<fieldset id="comp_{comp.id}" class="dsb-comp" style="{style};border:1px solid #ced4da;border-radius:4px;padding:8px 12px;" {dtype}>'
