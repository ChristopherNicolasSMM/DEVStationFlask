...
class Transaction(db.Model):
    ...
    )  # Design | Integration | DevOps | Admin
    ...
    # ── Controle de acesso ────────────────────────────────────────
    min_profile = db.Column(
        db.String(20), default="USER"
    )  # USER|PUSER|BANALYST|DEVELOPER|...
    is_active = db.Column(db.Boolean, default=True)
    is_standard = db.Column(db.Boolean, default=True)  # False = NDS_* (plugin)

    # ── Origem ────────────────────────────────────────────────────
    plugin_code = db.Column(
        db.String(100), nullable=True
    )  # código do plugin de origem (NDS_*)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "created_at": (
                self.created_at.strftime("%d/%m/%Y %H:%M") if self.created_at else None
            ),
        }
