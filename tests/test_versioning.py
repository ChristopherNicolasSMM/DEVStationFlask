...
class TestVersionCreate:
    def test_save_page_creates_auto_snapshot(self, client, page_id):
        r = client.post(f"/api/paginas/{page_id}/salvar", json={
            "components": [
                {"type": "label", "name": "lbl1", "x": 0, "y": 0,
                 "width": 100, "height": 30, "properties": {}, "events": {}, "rules": []}
            ]
        })
...
class TestVersionRestore:
    def test_restore_creates_pre_snapshot(self, client, page_id):
        ...
        # Restaura
        r2 = client.post(f"/api/versoes/{vid}/restaurar", json={"triggered_by": "tester"})
        assert r2.status_code == 200
        data = r2.get_json()
...
class TestVersionDiff:
    def test_diff_two_versions(self, client, page_id):
        ...
        client.post(f"/api/paginas/{page_id}/salvar", json={
            "components": [
                {"type": "label", "name": "compA", "x": 10, "y": 10,
                 "width": 100, "height": 30, "properties": {}, "events": {}, "rules": []}
            ]
        })
        ...
        client.post(f"/api/paginas/{page_id}/salvar", json={
            "components": [
                {"type": "button", "name": "compB", "x": 20, "y": 20,
                 "width": 100, "height": 40, "properties": {}, "events": {}, "rules": []}
            ]
        })
...
class TestPurgeSuggestions:
    def test_purge_suggestions_listed(self, client, page_id):
        ...
        for i in range(12):
            client.post(f"/api/paginas/{page_id}/salvar", json={
                "components": [
                    {"type": "label", "name": f"comp_{i}", "x": i*10, "y": 0,
                     "width": 100, "height": 30, "properties": {}, "events": {}, "rules": []}
                ]
            })
