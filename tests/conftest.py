...
from models import db as _db, Project, Page, ODataConnection
...
def _seed_test_data():
    from models import db, Project, Page, Component, Menu, Transaction
    from transactions.catalog import DS_TRANSACTIONS

    project = Project(name="Projeto Teste", description="Projeto para testes automatizados")
    _db.session.add(project)
    _db.session.flush()

    page = Page(project_id=project.id, name="Página Teste",
                slug="index", is_home=True, order=0)
    _db.session.add(page)
    _db.session.flush()

    comp = Component(
        page_id=page.id, type="button", name="btnTeste",
        x=100, y=100, width=120, height=40, z_index=1,
        properties={"text": "Teste"}, events={}, rules=[],
    )
