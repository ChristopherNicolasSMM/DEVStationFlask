...
class ODataScreenGenerator:
    ...
    def _gen_list(self, entity: dict, base_name: str):
        ...
        col_labels = [
            c.get("label", c.get("field", c)) if isinstance(c, dict) else c
            for c in raw_cols
        ]
    ...
    def _gen_form(self, entity: dict, base_name: str):
        ...
        for group in groups:
            ...
            for field in fields:
                ...
                ftype = (
                    field.get("type", "TEXT") if isinstance(field, dict) else "TEXT"
                ).upper()
                required = (
                    ...
                    rules.append(
                        {
                            ...
                            "params": {
                                "max": maxlen,
                                "message": f"Máximo {maxlen} caracteres.",
                            },
                        }
                    )
    ...
    def _default_columns(self, entity: dict) -> list:
        ...
        if not fields:
            return [{"field": "ID", "label": "ID"}, {"field": "Nome", "label": "Nome"}]
        return [
            {
